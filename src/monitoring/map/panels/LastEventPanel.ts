...
class LastEventPanelController implements ng.IController {
    ...
    public goToEvents() {
        if (this._lastEvents && this._lastEvents.length > 0) {
            ...
            this.$state.go(this.state || 'app.events');
        }
    }
    ...
    private openEvent(event: iqs.shell.OperationalEvent): void {
        ...
        if (nowDate.getTime() - eventDate.getTime() < 24 * 60 * 60 * 1000) {
            ...
            // this.$state.go('retrospective.events', {
            //     retro_date: eventDate.toISOString(),
            //     event_id: event.id
            // });
            window.location.href = window.location.origin + `/retrospective/index.html#/app/events?retro_date=${eventDate.toISOString()}&event_id=${event.id}`;
        }
    }
...
(() => {
    angular
        ...
        .component('iqsLastEventPanel', {
            ...
            templateUrl: 'monitoring/map/panels/LastEventPanel.html',
