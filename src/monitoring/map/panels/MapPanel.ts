...
interface IMonitoringMapPanelBindings {
    [key: string]: any;
}

const MonitoringMapPanelBindings: IMonitoringMapPanelBindings = {
    type: '<iqsType'
}

class MonitoringMapPanelChanges implements ng.IOnChangesObject, IMonitoringMapPanelBindings {
    [key: string]: ng.IChangesObject<any>;

    type: ng.IChangesObject<string>;
}

class MonitoringMapPanelController implements ng.IController {
    ...
    constructor(
        ...
        const runWhenReady = () => {
            ...
            this.markerOptions.popup = {
                ...
                templateUrl: 'monitoring/map/MapPopup.html',
            };
            ...
            this.zoneOptions.popup = {
                options: {
                    ...
                    events: {
                        onConfigureZone: (zone) => {
                            ...
                            window.location.href = window.location.origin + `/config_zones/index.html#/zones?zone_id=${zone.id}&state=edit`;
                        }
                    }
                },
                offset: {
                ...
                className: this.type === 'retro' ? 'pip-map-info-top-margin' : '',
                templateUrl: 'monitoring/map/ZonePopup.html',
            };
            ...
            this.dottracesOptions.popup = {
                ...
                className: this.type === 'retro' ? 'pip-map-info-top-margin' : '',
                templateUrl: 'monitoring/map/DotTracesPopup.html'
            }
            ...
            this.polylineOptions.popup = {
                ...
                templateUrl: 'monitoring/map/RoutePopup.html',
            };
    ...
    public goToGlobalMapSettings() {
        // this.$state.go('settings_system.location');
        window.location.href = window.location.origin + `/config_system/index.html#/organization/location`;
    }
    ...
    private onEditClick(model) {
        if (model.object_id) {
            ...
            window.location.href = window.location.origin + `/config_objects/index.html#/objects?object_id=${model.object_id}&edit=edit`;
        } else {
            // this.$state.go('settings_system.devices', { device_id: model.device_id, edit: 'edit' });
            window.location.href = window.location.origin + `/config_devices/index.html#/devices?device_id=${model.device_id}&edit=edit`;
        }

    }

    private onInfoClick(model) {
        // TODO #0000: dirty way, have to find more beautiful one
        if (model.object_id) {
            // this.$state.go('settings_system.objects', { object_id: model.object_id });
            window.location.href = window.location.origin + `/config_objects/index.html#/objects?object_id=${model.object_id}`;
        } else {
...
(() => {
    angular
        .module('iqsMonitoring.Map.Panel', [
            'iqsSearchDialog',
        ...
        .component('iqsMonitoringMapPanel', {
            bindings: MonitoringMapPanelBindings,
            templateUrl: 'monitoring/map/panels/MapPanel.html',
            controller: MonitoringMapPanelController,
            controllerAs: '$ctrl'
        })
})();
